...
class LoginWindow(ctk.CTk):
    def __init__(self, on_success):
        ...
        self.title("ACCESS RESTRICTED")
        self.geometry("400x300")
        self.configure(fg_color="#0d0208")
        self.resizable(False, False)

        # UI Hacker Style for Login
        self.label = ctk.CTkLabel(self, text="ENTER SECURITY CODE", 
                                 text_color="#00ff41", 
        ...
        self.login_button = ctk.CTkButton(self, text="VERIFY", 
                                         fg_color="#008f11", 
                                         hover_color="#00ff41",
                                         text_color="black",
        ...
        self.error_label = ctk.CTkLabel(self, text="", text_color="#ff0000")
        self.error_label.pack()
        
        self.password_entry.focus()

    def check_password(self):
        if self.password_entry.get() == "2026":
            self.destroy()
            self.on_success()
        else:
...
class AtmLogAnalyzer(ctk.CTk):
    ...
    def create_sidebar(self):
        ...
        self.btn_atm = ctk.CTkButton(self.sidebar_frame, corner_radius=8, height=40, border_spacing=10, 
                                     text="Analisa ATM",
                                     fg_color="transparent", text_color=self.hacker_green, 
                                     hover_color=self.hacker_green_dark,
                                     anchor="w", command=self.btn_atm_event)
        self.btn_atm.grid(row=2, column=0, sticky="ew", padx=10, pady=2)

        self.btn_crm = ctk.CTkButton(self.sidebar_frame, corner_radius=8, height=40, border_spacing=10, 
                                     text="Rekap CRM",
                                     fg_color="transparent", text_color=self.hacker_green, 
                                     hover_color=self.hacker_green_dark,
                                     anchor="w", command=self.btn_crm_event)
        self.btn_crm.grid(row=3, column=0, sticky="ew", padx=10, pady=2)

        self.btn_hitachi = ctk.CTkButton(self.sidebar_frame, corner_radius=8, height=40, border_spacing=10, 
                                         text="CRM Hitachi",
                                         fg_color="transparent", text_color=self.hacker_green, 
                                         hover_color=self.hacker_green_dark,
                                         anchor="w", command=self.btn_hitachi_event)
        self.btn_hitachi.grid(row=4, column=0, sticky="ew", padx=10, pady=2)

        self.btn_join = ctk.CTkButton(self.sidebar_frame, corner_radius=8, height=40, border_spacing=10, 
                                         text="Join File TXT",
                                         fg_color="transparent", text_color=self.hacker_green, 
                                         hover_color=self.hacker_green_dark,
                                         anchor="w", command=self.btn_join_event)
        self.btn_join.grid(row=5, column=0, sticky="ew", padx=10, pady=2)

        self.btn_advanced = ctk.CTkButton(self.sidebar_frame, corner_radius=8, height=40, border_spacing=10, 
                                         text="Analisa Lanjutan",
                                         fg_color="transparent", text_color=self.hacker_green, 
                                         hover_color=self.hacker_green_dark,
                                         anchor="w", command=self.btn_advanced_event)
    ...
    def select_frame_by_name(self, name):
        ...
        # Show selected frame
        if name == "atm":
            self.frame_atm.pack(fill="both", expand=True)
            self.header_label.configure(text="Analisa ATM")
        else:
            self.frame_atm.pack_forget()

        if name == "crm":
            self.frame_crm.pack(fill="both", expand=True)
            self.header_label.configure(text="Rekap CRM")
        else:
            self.frame_crm.pack_forget()

        if name == "hitachi":
            self.frame_hitachi.pack(fill="both", expand=True)
            self.header_label.configure(text="CRM Hitachi")
        else:
            self.frame_hitachi.pack_forget()

        if name == "join":
            self.frame_join.pack(fill="both", expand=True)
            self.header_label.configure(text="Join File TXT")
        else:
            self.frame_join.pack_forget()

        if name == "advanced":
            self.frame_advanced.pack(fill="both", expand=True)
            self.header_label.configure(text="Analisa Lanjutan")
        else:
    ...
    def run_vault_log(self):
        ...
        # Pola untuk pintu brankas (Safe/Vault)
        vault_patterns = [
        ...
        try:
            ...
            for i, line in enumerate(lines):
                ...
                if is_vault_open:
                    # Ambil jam dari baris tersebut atau baris sekitarnya
                    jam_match = re.search(r'(\d{2}:\d{2}:\d{2})', clean_line)
                    jam = jam_match.group(1) if jam_match else "N/A"
                    
                    # Jika jam tidak ada di baris yang sama, coba cari di baris sebelumnya/sesudahnya
                    if jam == "N/A":
                        for offset in range(-2, 3):
                            if 0 <= i + offset < len(lines):
                                nearby_line = re.sub(r'[\x00-\x1f\x7f-\xff\x1b]', '', lines[i+offset])
                                j_match = re.search(r'(\d{2}:\d{2}:\d{2})', nearby_line)
                                if j_match:
                                    jam = j_match.group(1)
                                    break
